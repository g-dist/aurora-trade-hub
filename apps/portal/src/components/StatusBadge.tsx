...
type StatusType = 'active' | 'in-stock' | 'low-stock' | 'on-order' | 'pending' | 'confirmed' | 'cancelled' | 'pre-order' | 'processing' | 'dispatched' | 'in-transit' | 'arrived' | 'delivered' | 'quote-sent' | 'negotiating' | 'expired' | 'pending-review' | 'limited' | 'completed' | 'rejected';
...
const statusConfig: Record<StatusType, { label: string; className: string }> = {
  ...
  'rejected': { label: 'Rejected', className: 'bg-status-red/15 text-status-red' },
};
